...
// MARK: - RightPanelView v7
//
// Cambios v6 → v7:
//   ✨ ADD: Tab "LoRAs" — panel completo de LoRAManager con lista, selección y pesos
//          ROADMAP: "Panel LoRA Manager (sdapi/v1/loras)" ahora conectado a la UI
//   🔁 UPD: PanelTab enum añade .lora con icono "slider.horizontal.below.rectangle"
...
struct RightPanelView: View {
    ...
    @StateObject var assetStore     = AssetStore.shared                 // NEW v6
    ...
    enum PanelTab: String, CaseIterable {
        ...
        case lora      = "LoRAs"     // NEW v7 — ROADMAP: Panel LoRA Manager
        ...
        var icon: String {
            ...
            case .lora:      return "slider.horizontal.below.rectangle"
    ...
    var tabContent: some View {
        ...
        case .ipadapter:
            ipAdapterTab
        case .lora:
            LoRAManagerTab()          // NEW v7 — ROADMAP: Panel LoRA Manager
        case .presets:
            presetsTab
        case .export:
            exportTab                    // NEW v6
    ...
    @ViewBuilder
    private var ipAdapterTabContent: some View {
        VStack(spacing: 12) {
            IPAdapterPanel()
    ...
    // MARK: - IC-Light Section (extracted to help type-checker)

    @ViewBuilder
    private var icLightSection: some View {
    ...
    @ViewBuilder
    private var icLightHeader: some View {
        HStack {
    ...
    @ViewBuilder
    private var icLightControls: some View {
        VStack(alignment: .leading, spacing: 8) {
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 6) {
    ...
    @ViewBuilder
    private var icLightPresets: some View {
    ...
    @ViewBuilder
    private var icLightPresetList: some View {
        ForEach(Array(ICLightEngine.builtInPresets.prefix(4))) { preset in
            icLightPresetRow(preset)
        }
    }

    @ViewBuilder
    private func icLightPresetRow(_ preset: ICLightEngine.LightingPreset) -> some View {
    ...
    @ViewBuilder
    private var icLightApplyButton: some View {
        ...
        HStack(spacing: 7) {
            ...
            if seed > 0 {
                ...
                Button(action: {
                    ...
                    else   { SeedManager.shared.addFavorite(seed: seed, label: String(parsedPrompt.prefix(40)), promptHint: parsedPrompt) }
                }) {
    ...
    @ViewBuilder private func animRing(_ i: Int, isGen: Bool) -> some View {
        Circle().stroke(Color(hex: "#7c6af7").opacity(0.22 - Double(i) * 0.06), lineWidth: 1.5)
            ...
            .scaleEffect(isGen ? 1.12 : 1.0)
            .animation(.easeInOut(duration: 1.1 + Double(i) * 0.25).repeatForever(autoreverses: true)
                .delay(Double(i) * 0.2), value: isGen)
    }

    var generatingAnim: some View {
        let isGen      = sdService.isGenerating
        let statusText = progressEngine.isGenerating ? progressEngine.statusLabel : sdService.progressText
        let ipEnabled  = ipAdapter.isEnabled
        let ipName     = ipAdapter.config.model.displayName
        return VStack(spacing: 16) {
            ZStack {
                ForEach(0..<3, id: \.self) { i in animRing(i, isGen: isGen) }
                ProgressView().scaleEffect(1.1).progressViewStyle(.circular)
            }
            Text(statusText).font(.system(size: 12, weight: .medium)).foregroundColor(.white.opacity(0.45))
            if ipEnabled {
                HStack(spacing: 4) {
                    Circle().fill(Color(hex: "#7c6af7")).frame(width: 5, height: 5)
                    Text("IP-Adapter \(ipName) activo").font(.system(size: 10)).foregroundColor(.secondary)
                }
            }
        }
    }
    ...
    private var stageInfo: (Color, String) {
        ...
        case .img2img:     return (.mint,    "Img2Img")
        case .upscaling:   return (.indigo,  "Upscaling")
        case .adetailer:   return (.pink,    "ADetailer")
        case .postprocess: return (.purple,  "Post‑proc")
        case .cleanup:     return (.brown,   "Cleanup")
        case .saving:      return (.teal,    "Saving")
        case .exporting:   return (.cyan,    "Exporting")
        case .done:        return (.green,   "Done ✓")
        case .error:       return (.red,     "Error")
        case .interrupted: return (.gray,    "Interrupted")
        }
    }
...
// MARK: - LoRAManagerTab (NEW v7)
// ROADMAP: "Panel LoRA Manager (sdapi/v1/loras)" — conectado a UI

struct LoRAManagerTab: View {

    @StateObject private var manager = LoRAManager.shared
    @State private var showWeightEditor: IdentifiableUUID? = nil
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 8) {
                ...
                Button(action: {
                    let url = UserDefaults.standard.string(forKey: "sd.baseURL") ?? "http://127.0.0.1:7860"
                    manager.configure(baseURL: url)
                    Task { await manager.fetchLoRAs() }
                }) {
            ...
            HStack(spacing: 6) {
                ...
                TextField("Buscar LoRA…", text: $manager.searchText)
                    ...
                    .foregroundColor(.white)
                Toggle("", isOn: $manager.filterFavorites)
                    .help("Solo favoritos")
                    .scaleEffect(0.75)
                    .frame(width: 36)
            }
            ...
            // ── Active LoRAs ────────────────────────────────────────────
            if !LoRAManager.shared.selectedLoRAs.isEmpty {
                VStack(alignment: .leading, spacing: 4) {
                    Text("ACTIVOS (\(manager.selectedLoRAs.count))")
                        .font(.system(size: 9, weight: .semibold))
                        .foregroundColor(Color(hex: "#3de3c0"))
                        .padding(.horizontal, 10).padding(.top, 8)

                    ForEach(manager.selectedLoRAs) { sel in
                        HStack(spacing: 8) {
                            Image(systemName: "checkmark.circle.fill")
                                .font(.system(size: 10))
                                .foregroundColor(Color(hex: "#3de3c0"))

                            Text(sel.lora.displayName)
                                .font(.system(size: 11))
                                .foregroundColor(.white)
                                .lineLimit(1)
                            ...
                            // Weight control
                            Text(String(format: "%.2f", sel.weight))
                                .font(.system(size: 10, design: .monospaced))
                                .foregroundColor(Color(hex: "#7c6af7"))
                                .onTapGesture { showWeightEditor = IdentifiableUUID(sel.id) }

                            Button(action: { manager.removeLoRA(id: sel.id) }) {
                    ...
                    Button("Limpiar selección") { manager.clearSelection() }
                        .font(.system(size: 10))
                        .foregroundColor(Color(hex: "#ef4444").opacity(0.8))
                        .buttonStyle(.plain)
                        .padding(.horizontal, 10).padding(.bottom, 6)
                }
                .background(Color(hex: "#3de3c0").opacity(0.05))

                Divider().background(Color.white.opacity(0.07))
            }

            // ── Available LoRAs ─────────────────────────────────────────
            if manager.availableLoRAs.isEmpty && !manager.isLoading {
                VStack(spacing: 10) {
                    Image(systemName: "slider.horizontal.below.rectangle")
                        .font(.system(size: 28))
                        .foregroundColor(.secondary.opacity(0.4))
                    Text("Sin LoRAs cargados")
                        .font(.system(size: 12))
                        .foregroundColor(.secondary)
                    Text("Pulsa ↺ para cargar desde A1111")
                        .font(.system(size: 10))
        ...
        .background(Color(red: 0.09, green: 0.09, blue: 0.12))
        // Weight editor popover
        .popover(item: $showWeightEditor) { wrapper in
            let id = wrapper.id
            if let sel = LoRAManager.shared.selectedLoRAs.first(where: { $0.id == id }) {
                loraWeightPopover(sel)
            }
        }
    }
    ...
    func loraRow(_ lora: LoRAEntry) -> some View {
        let selected = LoRAManager.shared.isSelected(lora)
        return HStack(spacing: 8) {
            // Favorite star
            Button { LoRAManager.shared.toggleFavorite(lora) } label: {
                Image(systemName: manager.isFavorite(lora) ? "star.fill" : "star")
                    .font(.system(size: 10))
                    .foregroundColor(manager.isFavorite(lora) ? Color(hex: "#f59e0b") : .secondary)
            }
            .buttonStyle(.plain)
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(lora.displayName)
                    ...
                    .foregroundColor(selected ? Color(hex: "#7c6af7") : .white)
                    .lineLimit(1)
                if let meta = lora.metadata,
                   let trigger = meta.triggerWords?.first {
                    Text(trigger)
                        .font(.system(size: 9))
            ...
            Button(action: {
                ...
                Image(systemName: selected ? "checkmark.circle.fill" : "plus.circle")
                    .font(.system(size: 14))
                    .foregroundColor(selected ? Color(hex: "#7c6af7") : .secondary)
            }
            .buttonStyle(.plain)
        }
        .padding(.horizontal, 10).padding(.vertical, 7)
        .background(selected ? Color(hex: "#7c6af7").opacity(0.08) : Color.clear)
    }

    // MARK: - Weight Popover

    func loraWeightPopover(_ sel: SelectedLoRA) -> some View {
        VStack(spacing: 12) {
            Text(sel.lora.displayName)
                .font(.system(size: 12, weight: .semibold))
                .foregroundColor(.white)
            ...
            HStack(spacing: 10) {
                Slider(
                    value: Binding(
                        get: { manager.selectedLoRAs.first(where: { $0.id == sel.id })?.weight ?? 0.8 },
                        set: { manager.updateWeight(id: sel.id, weight: $0) }
                    ),
                    in: 0.0...1.5,
                    step: 0.05
                )
                ...
                Text(String(format: "%.2f",
                     manager.selectedLoRAs.first(where: { $0.id == sel.id })?.weight ?? 0.8))
                    .font(.system(size: 11, design: .monospaced))
                    .foregroundColor(Color(hex: "#7c6af7"))
                    .frame(width: 36)
            }

            Text("Token: \(sel.a1111Token)")
                .font(.system(size: 9, design: .monospaced))
                .foregroundColor(.secondary)
                .lineLimit(1)
        }
    ...
    // MARK: - Bottom Controls alias (used in Extensions)
    var bottomControls: some View {
        if let img = sdService.generatedImage {
            return AnyView(bottomBar(img))
        }
