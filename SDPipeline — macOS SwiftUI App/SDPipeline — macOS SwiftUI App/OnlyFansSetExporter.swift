...
final class OnlyFansSetExporter: ObservableObject {
    ...
    func exportSet(
        ...
        // 6. ── ZIP (Nativo de macOS para Escalabilidad sin dependencias) ──
        ...
        if config.zipOutput {
            ...
            process.currentDirectoryURL = exportDir.deletingLastPathComponent()
            // -r = recurse, -q = quiet, -9 = max compression
            process.arguments = ["-r", "-q", "-9", zipPath, exportDir.lastPathComponent]
            ...
            do {
                ...
                if process.terminationStatus == 0 {
                    ...
                    print("⚠️ Advertencia: /usr/bin/zip devolvió el código \(process.terminationStatus)")
                }
            } catch {
                print("⚠️ Error al crear el ZIP nativo: \(error.localizedDescription)")
            }
        }
    ...
    private func writeManifest(to url: URL, assets: [GeneratedAsset], setTitle: String, cleanURLs: [URL], session: ContentSessionManager.ContentSession?) throws {
        ...
        if let session = session {
            ...
            manifest["platform"]     = session.platform.rawValue
        }
...
// MARK: - Stubs for missing dependencies
private extension LicenseLocalStore {
    func hasVerifiedLicense(for checkpoint: String) -> Bool { return true }
}
private extension ConsentTemplateManager {
    func hasActiveTemplate(for platform: ConsentTemplateManager.Platform) -> Bool { return !templates.isEmpty }
    func activeTemplateName(for platform: ConsentTemplateManager.Platform) -> String? { return templates.first?.name }
}
private extension PrivacyComplianceManager {
    var currentPolicyVersion: String { return privacyPolicy?.version ?? "1.0" }
}
