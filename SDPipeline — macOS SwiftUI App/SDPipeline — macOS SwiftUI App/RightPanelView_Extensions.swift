...
extension RightPanelView {
    ...
    var outputTabWithLivePreview: some View {
        VStack(spacing: 0) {
            ...
            // ── Controles inferiores (inline) ─────────────────────────────
            HStack(spacing: 8) {
                if let dur = sdService.generationDuration > 0 ? sdService.generationDuration : nil {
    ...
    // MARK: - Action Bar helpers (postGenActionBar defined in RightPanelView.swift)
    ...
    var currentRating: Int {
        guard let asset = AssetStore.shared.recentAssets.last else { return 0 }
        return Int(asset.rating)
    }

    // setRating defined in RightPanelView.swift

    // MARK: - Export Actions (wired to new engines)

    func exportCurrentToVault() {
        guard let asset = AssetStore.shared.recentAssets.last else { return }
        Task {
            do {
    ...
    func enqueueToExportBatch() {
        let pending = AssetStore.shared.recentAssets.prefix(1).map { $0 }
    ...
    func exportCurrentSession(_ session: ContentSessionManager.ContentSession) {
        let assets = AssetStore.shared.recentAssets.filter { $0.statusEnum == AssetStatus.approved }
    ...
    // exportTab defined in RightPanelView.swift — this is the extended export panel
    var exportTabExtended: some View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if let session = selectedSession {
                let approvedAssets = store.recentAssets.filter { $0.statusEnum == AssetStatus.approved }
                HStack(spacing: 6) {
