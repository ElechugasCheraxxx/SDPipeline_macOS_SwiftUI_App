...
final class VaultCryptoEngine: ObservableObject {
    ...
    func encrypt(_ plaintext: Data, using key: SymmetricKey? = nil) throws -> Data {
        let sealedBox = try AES.GCM.seal(plaintext, using: key ?? vaultKey)
    ...
    func decrypt(_ ciphertext: Data, using key: SymmetricKey? = nil) throws -> Data {
        let sealedBox = try AES.GCM.SealedBox(combined: ciphertext)
        return try AES.GCM.open(sealedBox, using: key ?? vaultKey)
    }
