...
final class TiledUpscalerEngine: ObservableObject {
    ...
    func upscale(
        ...
        if let assetID = asset.id, let outputData = try? Data(contentsOf: result.outputURL) {
            _ = try? AssetVersioningStore.shared.addVersion(
                assetID: assetID,
                imageData: outputData,
                tag: .upscaled,
                customLabel: versionTag,
                transformationNote: "Tiled upscale \(cfg.mode.rawValue) @ \(cfg.scaleFactor)x — \(result.tilesUsed) tiles"
            )
        }
