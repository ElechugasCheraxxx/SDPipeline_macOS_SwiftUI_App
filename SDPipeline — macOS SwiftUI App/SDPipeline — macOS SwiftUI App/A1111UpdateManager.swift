...
final class A1111UpdateManager: ObservableObject {
    ...
    func installUpdate() async throws {
        ...
        // Reinicio administrado por eventos en la vista, la instancia de SDService se inyecta usualmente
        NotificationCenter.default.post(name: NSNotification.Name("SDPipeline.restartWebUI"), object: nil)
    }
    ...
    private func notifyUpdateAvailable() {
        ...
        content.title = "SDPipeline Studio — Actualización disponible"
        content.body = "\(config.fork.rawValue) \(versionInfo.latest) está disponible. Instalado: \(versionInfo.current)"
