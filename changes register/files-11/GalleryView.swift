...
struct MetadataInspector: View {
    ...
    private func exportAsset() {
        guard let imagePath = asset.imagePath,
              let pngData   = try? Data(contentsOf: URL(fileURLWithPath: imagePath)),
              let exportDir = VaultManager.shared.exportURL
        else { exportMessage = "⚠️ No se pudo acceder a la imagen"; return }

        let previewDir = exportDir.appending(path: "Previews")
        try? FileManager.default.createDirectory(at: exportDir,   withIntermediateDirectories: true)
        try? FileManager.default.createDirectory(at: previewDir,  withIntermediateDirectories: true)

        let base = asset.baseName ?? "export_\(Int(Date().timeIntervalSince1970))"
        let config = ExportConfig(
            cleanOutputURL:   exportDir.appending(path:  "\(base)_clean.png"),
            previewOutputURL: previewDir.appending(path: "\(base)_preview.png"),
            watermark: WatermarkConfig(text: "© SDPipeline Studio", position: .bottomRight, opacity: 0.55),
            scrubMetadata: true
        )
        if let result = try? ExportEngine.export(imageData: pngData, config: config) {
            // Actualizar rutas en Core Data
            asset.cleanPath   = result.cleanURL.path
            asset.previewPath = result.previewURL.path
            try? AssetStore.shared.container.viewContext.save()
            exportMessage = "✓ Exportado a \(exportDir.lastPathComponent)"
            // Revelar en Finder
            NSWorkspace.shared.selectFile(result.cleanURL.path, inFileViewerRootedAtPath: "")
        } else {
            exportMessage = "⚠️ Error al exportar"
        }
