...
//   • PromptAutoCompleteEngine (índice de tokens SD en background)  ← NEW v3
//   • Todos los managers del roadmap
//
// CAMBIOS v3:
//   + Boot phase: autocomplete — indexa tokens Danbooru + LoRAs + Embeddings en background
//   + promptAutocomplete referenciado en AppEnvironment para acceso global
//   + Health check incluye estado del índice de autocompletado
...
final class AppEnvironment: ObservableObject {
    ...
    func boot() async {
        ...
        do {
            ...
            // ── Phase 9b: PromptAutoComplete Index ─────────────────────────
            advance(to: .autocomplete, progress: 0.69)
            Task.detached(priority: .utility) { [weak self] in
                // Recopilar nombres de LoRAs y embeddings para incluirlos en el índice
                let loraNames: [String]       = await LoRAManager.shared.loras.map { $0.name }
                let embeddingNames: [String]  = await EmbeddingsManager.shared.embeddings.map { $0.name }
                await PromptAutoCompleteEngine.shared.buildIndex(
                    loraNames:       loraNames,
                    embeddingNames:  embeddingNames
                )
                await MainActor.run {
                    let idxSize = PromptAutoCompleteEngine.shared.indexSize
                    self?.bootLog.append("[autocomplete] Índice: \(idxSize) tokens")
                }
            }
            log("PromptAutoComplete: indexado en background")
    ...
    var fullHealthStatus: String {
        ...
        let acIdx = promptAutocomplete.indexSize
        if acIdx == 0 {
            issues.append("Índice autocomplete vacío (indexando…)")
        }
        return issues.isEmpty ? "✅ Sistema saludable (\(acIdx) tokens indexados)" : "⚠️ " + issues.joined(separator: " · ")
    }
}
