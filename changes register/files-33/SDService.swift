...
//
// Cambios v3 → v4:
//   ✨ configureSession() — timeout y URLSession configurables
//   ✨ generateWithBatchRetry() — wrapper con PipelineRetryPolicy
//   ✨ resetState() — reset completo del estado
//   ✨ cancelProgressPoll() — acceso público al poll task
//   ✨ interruptAndReset() — interrupción + cleanup combinados
...
class SDService: ObservableObject {
    ...
    /// Configura timeouts de la sesión URLSession para generaciones largas.
    func configureSession(requestTimeout: TimeInterval = 600, resourceTimeout: TimeInterval = 600) {
        ...
        // Note: URLSession.shared no es reemplazable; configurar en App init o usar sesión privada.
    ...
    // MARK: - Interrupt + Reset (combined)

    /// Interrumpe la generación en A1111 Y resetea el estado local.
    ...
    // MARK: - cancelProgressPoll (public)
    ...
    // MARK: - generateWithBatchRetry (NEW v4)

    /// Genera con reintentos usando PipelineRetryPolicy.
    /// Wrapper sobre la lógica en Models_Extended_Patch.swift para uso desde BatchEngine.
