...
extension ADetailerEngine {
    ...
    func buildAlwaysOnScripts() -> [String: Any] {
        ...
        args.append([
            "ad_model":                    config.faceModel,
            "ad_prompt":                   config.facePrompt,
            "ad_negative_prompt":          config.faceNegativePrompt,
            "ad_confidence":               config.faceConfidence,
            "ad_mask_blur":                config.faceMaskBlur,
            "ad_denoising_strength":       config.faceDenoisingStrength,
            "ad_inpaint_full_res":         config.inpaintFullRes,
            "ad_inpaint_full_res_padding": config.inpaintPadding
        ])

        if config.handPassEnabled {
            args.append([
                "ad_model":               config.handModel,
                "ad_prompt":              config.handPrompt,
                "ad_confidence":          config.handConfidence,
                "ad_denoising_strength":  config.handDenoisingStrength
            ])
        }
