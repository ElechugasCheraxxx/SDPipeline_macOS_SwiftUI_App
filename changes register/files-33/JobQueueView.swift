...
//
// Cambios v1 → v2:
//   ✨ Panel de tabs: En Cola / Ejecutando / Completados / Fallidos
//   ✨ Barra de ETA + progreso global de la cola
//   ✨ Cancelar todos los jobs en cola (cancelAllQueued)
//   ✨ Reintentar todos los fallidos de un solo clic
//   ✨ Control de concurrencia (maxConcurrent 1–4 workers)
//   ✨ Detalle expandible por job (prompt, settings, errorLog)
//   ✨ Indicador de prioridad con colores
//   ✨ Batch grouping — jobs del mismo batchID se agrupan visualmente
//   ✨ Barra de búsqueda para filtrar jobs por nombre
            ...
            if filtered.isEmpty {
                ...
                ScrollView {
                    LazyVStack(spacing: 2) {
                        ...
                        ForEach(grouped, id: \.batchID) { group in
...
// MARK: - GenerationJob.durationLabel (bridge if missing)

private extension JobQueueManager.GenerationJob {
    var durationLabel: String {
        switch status {
        case .running:
            guard let s = startedAt else { return "corriendo…" }
            let secs = Int(-s.timeIntervalSinceNow)
            return "\(secs)s corriendo"
        case .done, .cancelled, .failed:
            guard let s = startedAt, let f = finishedAt else { return status.rawValue }
            let secs = Int(f.timeIntervalSince(s))
            return "\(secs)s · \(status.rawValue)"
        case .queued:
            return "en cola · \(createdAt.relativeLabel)"
        }
    }
}
