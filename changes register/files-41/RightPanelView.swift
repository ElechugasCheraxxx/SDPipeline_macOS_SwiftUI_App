...
struct LoRAManagerTab: View {
    ...
    @State private var showWeightEditor: UUID? = nil
    ...
    var body: some View {
        ...
        .popover(item: $showWeightEditor) { id in
            if let sel = manager.selectedLoRAs.first(where: { $0.id == id }) {
    ...
    func loraRow(_ lora: LoRAEntry) -> some View {
        let selected = manager.isSelected(lora)
