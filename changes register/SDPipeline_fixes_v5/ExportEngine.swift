...
// MARK: - BatchExportConfig / BatchExportResult are nested inside ExportEngine (see below)
...
final class ExportEngine: ObservableObject {
    ...
    struct BatchExportConfig {
        var preset:           ExportPreset                  = ExportPreset(name: "batch")
        var maxConcurrent:    Int                           = 2
        var continueOnError:  Bool                          = true
        var progressCallback: ((Int, Int) -> Void)?        = nil
    }

    struct BatchExportResult {
        let results:       [(GeneratedAsset, ExportResult)]
    ...
    /// Convenience overload — uses default BatchExportConfig.
    @MainActor
    func exportBatch(assets: [GeneratedAsset]) async -> BatchExportResult {
        await exportBatch(assets: assets, config: BatchExportConfig())
    }
    ...
    func exportBatch(
        ...
        await withTaskGroup(of: (GeneratedAsset, Result<ExportResult, Error>).self) { group in
            for asset in assets {
                group.addTask {
                    await semaphore.wait()
                    defer { Task { await semaphore.signal() } }
                    do {
                        let result = try await self.export(asset: asset, preset: config.preset)
            ...
            var completed = 0
            for await (asset, outcome) in group {
                completed += 1
                config.progressCallback?(completed, assets.count)
                switch outcome {
                case .success(let r): results.append((asset, r))
                case .failure(let e):
