...
struct SettingsView: View {
    ...
    var exportSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("Incrustados en la versión limpia (clean) de cada imagen exportada. El preview con watermark no incluye metadatos de autoría.")
                .font(.system(size: 11))
                .foregroundColor(.secondary)
                .fixedSize(horizontal: false, vertical: true)
            settingsGroup {
    ...
    var wildcardsSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            sectionHeader("Wildcards Dinámicos")
            Text("Usa __nombre__ en cualquier prompt para insertar un término aleatorio del grupo. Los grupos built-in son de solo lectura; los custom se guardan en el Vault.")
                .font(.system(size: 11))
                .foregroundColor(.secondary)
                .fixedSize(horizontal: false, vertical: true)
            WildcardEditorView()
    ...
    var auditSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            sectionHeader("Audit Log Cifrado (AES-GCM)")
