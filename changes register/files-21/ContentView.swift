...
"""
    ...
    func generate() {
        ...
        // Inyectar personaje activo + Wildcards + LoRAs seleccionados
        ...
        // ── Safety validation ANTES de enviar a SD ──────────────────────
        let safetyResult = PromptSafetyFilter.validatePrompt(
            positive: finalPrompt,
            negative: finalNegative
        )
        PromptSafetyFilter.logResult(safetyResult, prompt: finalPrompt)
        if safetyResult.isBlocked {
            sdService.errorMessage = safetyResult.userMessage ?? "Prompt bloqueado por filtro de seguridad."
            return
        }
        // ────────────────────────────────────────────────────────────────
        ...
        Task {
            ...
            // ── Benchmark automático por modelo ─────────────────────────
            ...
            // ── NSFW post-generación → ZK log cifrado ───────────────────
            ...
            // ── Auto-save prompt en PromptVersioningStore ────────────────
