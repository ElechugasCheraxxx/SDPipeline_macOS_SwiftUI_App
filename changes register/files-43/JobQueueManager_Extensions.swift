...
extension JobQueueManager {
    ...
    func enqueueAfter(
        ...
        let jobSettings = JobQueueManager.JobSettings(
            baseURL:          settings.sdBaseURL,
            autoExport:       true,
            autoNSFWCheck:    settings.autoRunNSFWCheck,
            postProcessing:   settings.autoRunPostProd
        )
        let job = GenerationJob(name: name, request: request, settings: jobSettings, priority: priority)
        enqueue(job)
        Task {
...
extension JobQueueManager {
    ...
    func enqueuePipeline(
        ...
        let genSettings = JobQueueManager.JobSettings(
            baseURL:          settings.sdBaseURL,
            autoExport:       true,
            autoNSFWCheck:    settings.autoRunNSFWCheck,
            postProcessing:   settings.autoRunPostProd
        )
        let genJob  = GenerationJob(name: genName, request: request, settings: genSettings, priority: .high)
        enqueue(genJob)
        ...
        if withPostProc {
            ...
            let ppJob = GenerationJob(name: "Post-process: \(label)", request: ppReq,
                                      settings: genSettings, priority: .normal)
            enqueue(ppJob)
...
extension JobQueueManager {
    func enqueueForAsset(_ asset: GeneratedAsset, type: JobType = .postProcess) -> GenerationJob {
        ...
        _ = SDRequest(prompt: "[asset:\(asset.id?.uuidString ?? "?")]")
