    ...
    private func injectTokens(_ tokens: [Token]) {
        for token in tokens {
            ...
            for i in 1...max(1, prefixLen) {
                ...
                if self.tokenIndex[prefix] == nil { self.tokenIndex[prefix] = [] }
                if self.tokenIndex[prefix]!.count < 200 {
                    self.tokenIndex[prefix]!.append(token)
                }
            }
        }
...
final class SidecarJSONManager {
    ...
    func write(for asset: GeneratedAsset, imageURL: URL) throws {
        let sidecarURL = imageURL.deletingLastPathComponent()
            .appendingPathComponent(asset.baseName ?? asset.id?.uuidString ?? UUID().uuidString)
            .appendingPathExtension("json")
        var sidecar = SidecarJSON(from: asset)
        // SidecarJSON fully initialized via init(from: asset)
...
// JobSettings.default y GenerationJob.init — ya definidos en JobQueueManager.swift

// MARK: - NSFWDetector detect bridge (imagePath optional)
...
extension NSFWDetector {
    func detect(
        ...
        await detect(prompt: prompt, image: image, baseURL: baseURL)
    }
}


// MARK: - BatchEngine.BatchItem default init (v2)

// BatchEngine.BatchItem init defined in JobQueueManager_Extensions.swift

// MARK: - PipelineRetryPolicy UserDefaults factory

// PipelineRetryPolicy.userConfigured defined in JobQueueManager_Extensions.swift

// GenerationSettings.sdBaseURL — ya definido en Models.swift como stored property
