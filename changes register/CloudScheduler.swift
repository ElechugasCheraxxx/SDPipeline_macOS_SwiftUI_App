...
//   • Round-robin y balanceo por carga de VRAM
//   • Failover automático entre nodos
//   • Job queue distribuido con prioridades
//
// ROADMAP: "Distribución de tareas en múltiples GPUs/nube" (🟢 LARGO PLAZO)
...
final class CloudScheduler: ObservableObject {
    ...
    struct ComputeNode: Identifiable, Codable, Equatable {
        ...
        var name:           String
        var baseURL:        String        // http://192.168.1.5:7860 or localhost:7860
        var apiKey:         String        = ""
        var type:           NodeType      = .local
        var priority:       Int           = 5       // 1–10, higher = preferred
        var maxVRAM:        Int           = 8       // GB
        var status:         NodeStatus    = .unknown
        var currentJobs:    Int           = 0
        var responseTimeMs: Double        = 0       // Last ping latency
        var successRate:    Double        = 1.0     // 0–1
    ...
    struct CloudProvider: Identifiable, Codable {
        ...
        var endpoint:   String = ""
        var model:      String = ""
        var costPerJob: Double = 0   // USD estimate
    ...
    @Published var nodes: [ComputeNode] { get { config.nodes } set { config.nodes = newValue } }
    ...
    struct ClusterStats {
        var totalNodes:    Int
        var onlineNodes:   Int
        var totalVRAM:     Int     // GB
        var activeJobs:    Int
        var jobsPerHour:   Double
        var avgLatencyMs:  Double
        var totalCost:     Double  // USD today
    }
    ...
    private func updateClusterStats() {
        ...
        clusterStats = ClusterStats(
            ...
            totalCost:    0   // Would require provider API calls
        )
    }
    ...
    /// Añade todos los puertos de A1111 que responden en localhost
...
// MARK: - SDRequest Codable conformance bridge

// CloudScheduler requires SDRequest to be Encodable for remote submission.
// If SDRequest is already Codable, this is a no-op.
// If not, this extension provides the minimal encoding for remote dispatch.
extension SDRequest: Encodable {
    // Uses the existing Codable conformance from Models.swift.
    // This extension exists to document the requirement explicitly.
}
