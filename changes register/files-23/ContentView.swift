...
"""
    ...
    @State private var showXYPlot:     Bool    = false   // ← NEW: XYPlot sheet
    @State private var showNewSession: Bool    = false   // ← NEW: Nueva sesión sheet
    @State private var validationMsg:  String? = nil     // ← NEW: ValidationReport feedback
    ...
    var body: some View {
        ...
        // NEW: XY Plot sheet — disparado por menú Pipeline → X/Y Plot…
        ...
        // NEW: Nueva sesión — disparado por menú Sesión → Nueva sesión…
        .sheet(isPresented: $showNewSession) {
            NewSessionSheet { title, category in
                _ = ContentSessionManager.shared.newSession(
                    title:    title,
                    tag:      title.lowercased().replacingOccurrences(of: " ", with: "_"),
                    category: category
                )
                showNewSession = false
            }
        }
        // ── Notification listeners (desde menú de la app) ────────────────
        .onReceive(NotificationCenter.default.publisher(for: .showXYPlot)) { _ in
            showXYPlot = true
        }
            ...
            if let session = sessionManager.activeSession {
                HStack(spacing: 6) {
                    ...
                    Button(action: { showNewSession = true }) {
                        Image(systemName: "plus.circle")
                            .font(.system(size: 11))
                            .foregroundColor(.secondary)
                    }.buttonStyle(.plain).help("Nueva sesión")
                }
                ...
                Button(action: { showNewSession = true }) {
                    HStack(spacing: 5) {
                        Image(systemName: "plus.circle")
                            .font(.system(size: 10))
                        Text("Iniciar sesión de contenido")
                            .font(.system(size: 11))
                    }
                    .foregroundColor(.secondary)
                    .frame(maxWidth: .infinity, alignment: .leading)
    ...
    var webuiStatusBar: some View {
        ...
        HStack(spacing: 6) {
            ...
            }.buttonStyle(.plain).foregroundColor(.secondary).help("Show webui.sh log")
    ...
    var logSheet: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            ScrollViewReader { proxy in
                ...
                .onChange(of: sdService.webuiLog) { _, _ in
    ...
    // MARK: - Hires Fix Section

    var hirесFixSection: some View {
    ...
    func parseJSON() {
        ...
        do {
            ...
            // NEW: Validación inmediata al parsear — feedback al usuario
    ...
    func generate() {
        ...
        // NEW: usar ValidationReport estructurado
        let report = PipelineConnector.validateBeforeGenerate(
    ...
    func browseWebUIScript() {
        ...
        if panel.runModal() == .OK, let url = panel.url {
            settings.webuiScriptPath = url.path
        }
    }
        ...
        VStack(spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Picker("", selection: $category) {
                    ForEach(ContentSessionManager.ContentSession.SessionCategory.allCases, id: \.self) { cat in
...
// MARK: - XYPlotSheetView
// Wrapper ligero sobre XYPlotEngine para dispararlo desde menú o shortcut.

struct XYPlotSheetView: View {
    let basePrompt:   String
    let baseNegative: String
    let baseURL:      String
    let width:        Int
    let height:       Int
