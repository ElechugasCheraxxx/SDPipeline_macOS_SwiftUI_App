...
final class SandboxManager: ObservableObject {
    ...
    func launchSD(scriptPath: String) async throws {
        ...
        // --api   → habilita la REST API en /sdapi/v1/
        // --listen → acepta conexiones en todas las interfaces (incluye 127.0.0.1)
        // --xformers → optimizaciones de memoria para GPU
        let isAppleSilicon = GPUMonitor.shared.isAppleSilicon
        var launchArgs = [scriptPath, "--api", "--listen", "--xformers"]
        if isAppleSilicon {
            launchArgs += ["--precision", "full", "--no-half",
...
extension AppHardeningManager {
    ...
    func checkSDProcessIsolationLive() async -> SecurityCheckResult {
        let status = SandboxManager.shared.isolationStatus
