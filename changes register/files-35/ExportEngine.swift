...
final class ExportEngine: ObservableObject {
    ...
    func export(
        ...
        let sha256Clean = sha256Hex(of: cleanData)
    ...
    struct BatchExportConfig {
        ...
        var progressCallback: ((Int, Int) -> Void)? = nil  // (completed, total)
    }
    ...
    func exportToDestination(
        ...
        try cleanData.write(to: cleanURL, options: .atomic)
        let sha256 = sha256Hex(of: cleanData)
        ...
        if preset.addWatermark {
            ...
            try cleanData.write(to: previewURL, options: .atomic)
        }
        return sha256
    }
