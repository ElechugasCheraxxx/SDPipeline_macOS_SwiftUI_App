...
extension ContentView {
    ...
    var exportQuickSection: some View {
        if let image = sdService.generatedImage, !sdService.isGenerating {
            VStack(alignment: .leading, spacing: 8) {
    ...
    func exportCurrentImage(withWatermark: Bool) {
        ...
        Task {
            do {
                let result = try await ExportEngine.shared.export(asset: asset, addWatermark: withWatermark)
    ...
    var pipelineFlagsSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(spacing: 6) {
                ...
                ), description: "Control pose/depth/edge · \(ControlNetEngine.shared.units.filter { $0.enabled }.count) unidades activas")
            ...
            if let activeChar = characterEngine.activeCharacter {
                HStack(spacing: 8) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text(activeChar.modelName ?? "Sin modelo asignado")
...
// MARK: - GenerationSettings extensions (v4)

extension GenerationSettings {
    var autoRetryOnError: Bool {
        get { UserDefaults.standard.bool(forKey: "gen.autoRetryOnError") }
        set { UserDefaults.standard.set(newValue, forKey: "gen.autoRetryOnError") }
    }
    ...
    var autoRunNSFWCheck: Bool {
        get { UserDefaults.standard.bool(forKey: "gen.autoRunNSFWCheck") }
        set { UserDefaults.standard.set(newValue, forKey: "gen.autoRunNSFWCheck") }
    }
...
// MARK: - SDService generation duration helper

extension SDService {
    var generationDuration: Double {
        get { UserDefaults.standard.double(forKey: "sdservice.lastDuration") }
    }
}
