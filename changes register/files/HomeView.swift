...
// ══════════════════════════════════════════════════════
//  HomeView.swift — Features · Home · Presentation
//  Vista principal. Solo compone componentes, sin lógica.
// ══════════════════════════════════════════════════════
...
struct HomeView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            // ── Status ────────────────────────────────
            StatusBadge(text: viewModel.state.statusText)
        }
        ...
        .shadow(color: DSColors.brandPrimary.opacity(0.2), radius: 24, x: 0, y: 0)
    }
}

// ── Logo (reutilizable aquí hasta moverlo al DesignSystem/Branding) ──
    ...
    var body: some View {
        ZStack {
            ForEach(0..<4, id: \.self) { i in
                WingLine(index: i)
                    .stroke(style: StrokeStyle(lineWidth: CGFloat(2.8 - Double(i) * 0.5), lineCap: .round))
