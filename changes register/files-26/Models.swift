...
struct SDProgressResponse: Codable {
    ...
    let current_image: String?    // base64 live preview (enable_preview in A1111 settings)
    let textinfo: String?
...
struct GenerationSettings {
    ...
    // Post-generation pipeline flags
...
// MARK: - GeneratedAsset (Core Data NSManagedObject)
...
public class GeneratedAsset: NSManagedObject {
    ...
    @NSManaged public var imagePath:       String?    // Relative path from vault root
    @NSManaged public var cleanPath:       String?    // Clean export (no metadata)
    @NSManaged public var previewPath:     String?    // Watermarked preview
    ...
    @NSManaged public var loraWeightsJSON: String?    // JSON encoded [String: Double]

    // Integrity
    @NSManaged public var sha256:          String?

    // Curation
    @NSManaged public var rating:          Int32      // 0-5 stars
    ...
    @NSManaged public var nsfwScore:       Double
    @NSManaged public var hasCleanExport:  Bool
    @NSManaged public var hasPreview:      Bool
    @NSManaged public var hasSidecar:      Bool

    // Computed helpers
    var statusEnum: AssetStatus {
        get { AssetStatus(rawValue: status ?? "") ?? .raw }
...
// MARK: - NSManagedObjectModel programmatic definition

extension NSManagedObjectModel {

    static let sdPipelineStudioModel: NSManagedObjectModel = {
        ...
        entity.properties = [
            ...
            attr("version",         .integer32AttributeType),
            ...
            attr("seed",            .integer64AttributeType),
            attr("steps",           .integer32AttributeType),
            attr("cfgScale",        .doubleAttributeType),
            attr("samplerName",     .stringAttributeType),
            attr("width",           .integer32AttributeType),
            attr("height",          .integer32AttributeType),
            ...
            attr("rating",          .integer32AttributeType),
            ...
            attr("isNSFW",          .booleanAttributeType),
            attr("nsfwScore",       .doubleAttributeType),
            attr("hasCleanExport",  .booleanAttributeType),
            attr("hasPreview",      .booleanAttributeType),
            attr("hasSidecar",      .booleanAttributeType),
        ]
