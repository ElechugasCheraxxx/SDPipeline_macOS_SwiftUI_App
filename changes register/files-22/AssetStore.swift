...
final class AssetStore: ObservableObject {
    ...
    func saveAsset(
        ...
        // ── Esteganografía (en background thread para no bloquear UI) ──────────
        let finalPNGData: Data = await Task.detached(priority: .userInitiated) {
            // SteganographyEngine.embed NO es async — opera síncronamente en background
            return SteganographyEngine.shared.embed(
                image:      image,
                assetID:    assetID,
