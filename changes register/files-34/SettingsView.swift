        ...
        VStack(alignment: .leading, spacing: 20) {
            ...
            infoCard {
                ...
                Button("Re-indexar ahora") {
                    Task {
                        let loras = await LoRAManager.shared.loras.map { $0.name }
                        let embs  = await EmbeddingsManager.shared.embeddings.map { $0.name }
                        await PromptAutoCompleteEngine.shared.buildIndex(loraNames: loras, embeddingNames: embs)
                    }
                }
                .buttonStyle(ActionChipStyle())
            }
            ...
            infoCard {
                ...
                paramRow("Tasa de éxito",    "\(JobQueueManager.shared.formattedSuccessRate)")
    ...
    var controlNetSection: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            infoCard {
                ...
                paramRow("Unidades activas",   "\(engine.units.filter { $0.enabled }.count) / \(engine.units.count)")
                paramRow("Modelos instalados", "\(engine.availableModels.count)")

                if engine.availableModels.isEmpty && engine.units.isEmpty {
                    HStack(spacing: 6) {
            ...
            infoCard {
                ...
                if engine.units.isEmpty {
                    ...
                    ForEach(engine.units.indices, id: \.self) { i in
                        ...
                        HStack(spacing: 10) {
                            ...
                            VStack(alignment: .leading, spacing: 2) {
                                Text("Unidad \(i+1) · \(unit.preprocessor.rawValue)")
                                    .font(.system(size: 11, weight: .medium)).foregroundColor(.white)
            ...
            Button("Recargar modelos desde A1111") {
                Task { await engine.fetchModels(baseURL: UserDefaults.standard.string(forKey: "sd.baseURL") ?? "http://127.0.0.1:7860") }
            }
            .buttonStyle(ActionChipStyle())
        }
    }
