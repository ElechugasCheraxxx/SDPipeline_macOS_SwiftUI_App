...
//
// Features:
//   • Export automático de assets aprobados al DAM seleccionado
//   • Sincronización de ratings y tags bidireccional (donde soportado)
//   • Smart Albums / Collections automáticos por personaje y sesión
//   • Watch folder integration para importación automática
//   • Sidecar XMP para compatibilidad universal
//
// ROADMAP: "DAM externo (Eagle/DigiKam)" (🟢 LARGO PLAZO)
...
final class DAMBridge: ObservableObject {
    ...
    struct DAMConfig: Codable {
        ...
        enum DAMType: String, CaseIterable, Codable {
            ...
            var icon: String {
                switch self {
                case .eagle:       return "eagle"     // SF Symbols doesn't have eagle, use text
    ...
    // MARK: - Eagle Integration (API-based)

    private func exportToEagle(asset: GeneratedAsset) async throws -> URL {
        // Eagle runs a local API on port 41595
        ...
        // Create Smart Album for character if enabled
    ...
    // MARK: - DigiKam Integration (SQLite-based)

    private func exportToDigiKam(asset: GeneratedAsset) async throws -> URL {
        // DigiKam uses a SQLite DB — we copy image to its albums directory
        // and add a sidecar XMP with metadata
        ...
        // Write XMP sidecar for DigiKam to pick up
    ...
    private func exportToWatchFolder(asset: GeneratedAsset, prefix: String = "") async throws -> URL {
        ...
        if watchPath.isEmpty {
            // Default: use vault Export/WatchFolder
    ...
    private func exportToLightroom(asset: GeneratedAsset) async throws -> URL {
        // Lightroom Classic doesn't have a public API.
        // Best approach: copy to a folder Lightroom is watching (auto-import folder).
        // User configures the auto-import folder in Lightroom preferences.
    ...
    func setupWatchFolder(at url: URL) {
        ...
        // Production implementation uses DispatchSourceFileSystemObject or FSEventStream
    }
    ...
    private func formatAlbumName(for asset: GeneratedAsset) -> String {
        ...
        name = name.replacingOccurrences(of: "{model}", with: asset.modelName ?? "SD")
        return name
    }
...
extension GeneratedAsset {
    var characterName: String? {
        // Derived from tags or prompt keywords
        return nil  // Full implementation reads from CharacterEngine
    }
    var modelName: String? { modelCheckpoint }
}
