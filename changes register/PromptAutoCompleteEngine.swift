...
final class PromptAutoCompleteEngine: ObservableObject {
    ...
    private func extractHistoryTokens(into tokens: inout [Token]) async {
        let recentPrompts = PromptDatabase.shared.recentPrompts(limit: 200)
        var tokenCounts: [String: Int] = [:]

        for prompt in recentPrompts {
            let parts = prompt.promptText.components(separatedBy: ",")
            for part in parts {
    ...
    private func loraTokens() -> [Token] {
        return LoRAManager.shared.loraList.map { lora in
            Token(
                text:      lora.name,
                category:  .lora,
                score:     0.9,
    ...
    private func embeddingTokens() -> [Token] {
        return EmbeddingsManager.shared.embeddings.map { emb in
            Token(
                text:      emb.name,
                category:  .embedding,
                score:     0.85,
