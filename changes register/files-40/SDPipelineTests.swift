...
//   ✨ ADD: SUITE 19 — ControlNetUnitTests     (scriptPayload, codable)
...
final class ControlNetUnitTests: XCTestCase {
    ...
    func test_scriptPayload_requiredKeys() {
        let payload = ControlNetUnit().scriptPayload
        for key in ["enabled","model","module","weight","guidance_start","guidance_end","pixel_perfect"] {
            XCTAssertNotNil(payload[key], "Missing key: \(key)")
        }
    }

    func test_scriptPayload_imageIncluded() {
        var unit = ControlNetUnit(); unit.imageBase64 = "imgdata"
        XCTAssertEqual(unit.scriptPayload["image"] as? String, "imgdata")
    }

    func test_scriptPayload_imageAbsentWhenNil() {
        XCTAssertNil(ControlNetUnit().scriptPayload["image"])
    }
