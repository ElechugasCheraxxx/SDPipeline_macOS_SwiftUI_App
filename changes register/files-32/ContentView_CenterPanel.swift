...
// Sustituye los EmptyView() del ContentView original.
//
// Secciones:
//   • promptSection         — prompt positivo/negativo con PromptBuilder toggle
//   • generationParamsSection — steps, CFG, sampler, dimensiones
//   • modelSection          — checkpoint picker + VAE
//   • hiresSection          — Hi-Res fix completo
//   • seedSection           — seed, randomize, favoritos
//   • loraSection           — LoRAs con sliders de peso
//   • characterSection      — personaje activo + IP-Adapter indicator
...
extension ContentView {
    ...
    var characterSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let char = characterEngine.activeCharacter {
                HStack(spacing: 10) {
                    // Thumbnail
                    if let img = NSImage(contentsOfFile: char.baseImagePath ?? "") {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        ...
                        // IP-Adapter toggle para personaje
                        Button(action: {
                    ...
                    Spacer()
                    Button(action: { characterEngine.clearActiveCharacter() }) {
    ...
    // MARK: - Post-Generation Pipeline Flags (updated)
...
// MARK: - GenerationSettings available options
...
// MARK: - IPAdapterEngine.isEnabled stored var

extension IPAdapterEngine {
    var isEnabled: Bool {
        get { config.enabled }
        set { config.enabled = newValue }
    }
}
