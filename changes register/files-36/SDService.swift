...
class SDService: ObservableObject {
    ...
    func generate(request: SDRequest, baseURL: String) async {
        ...
        do {
            ...
            ZeroKnowledgeLog.shared.write(
                category: .generation,
                message: "Generación completada — seed:\(lastSeed) pasos:\(request.steps) duración:\(String(format: "%.1f", generationDuration))s modelo:\(request.overrideSettings?.sdModelCheckpoint ?? settings?.checkpoint ?? "-")"
            )
...
// MARK: - SDProgressResponse (v4 compat model)

private struct SDProgressResponse: Decodable {
    let progress:      Double
