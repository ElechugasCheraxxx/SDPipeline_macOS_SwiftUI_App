...
//
// Features:
//   • Temas predefinidos (Dark Studio, Neon Night, Amber Classic, Arctic)
//   • Editor de tema personalizado con selector de color
//   • Export/Import de temas como JSON
//   • Persistencia en UserDefaults + Backup en vault
//   • Integración con AppEnvironment via @EnvironmentObject
//
// ROADMAP: "Personalización temática de UI" (🟡 MEDIO PLAZO)
...
final class ThemeManager: ObservableObject {
    ...
    struct AppTheme: Codable, Identifiable, Equatable {
        ...
        // Background layers
        var bgBase:       String = "#0d0d14"   // Deepest dark
        var bgSurface:    String = "#13131e"   // Panel background
        var bgCard:       String = "#1a1a2e"   // Card / elevated
        var bgInput:      String = "#111118"   // Text field
    ...
    private func applyToAppearance() {
        // Force dark appearance always (studio app)
        NSApp.appearance = NSAppearance(named: .darkAqua)

        // Inject accent color into NSColor.controlAccentColor via NSColorPanel
        // (Limited by AppKit; this is a best-effort approach)
        if let accentNS = NSColor(hex: activeTheme.accentPrimary) {
    ...
    /// Inject as @EnvironmentObject or use directly via ThemeManager.shared.activeTheme
    var t: AppTheme { activeTheme }
}

// MARK: - NSColor hex init (theme convenience)
...
extension View {
    /// Injects the active theme into the SwiftUI environment.
